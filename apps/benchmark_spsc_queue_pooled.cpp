...
int main(int argc, char* argv[]) {
    ...
    const std::uint64_t pool_capacity = argc > 5 ? parse_u64_arg(argv[5], num_messages) : num_messages;
    const auto queue_capacity = static_cast<std::size_t>(queue_capacity_arg);

    std::cout << "=== SPSC Ring Buffer Pooled Order Book Benchmark ===\n";
    ...
    PooledOrderBook book(static_cast<std::size_t>(pool_capacity));
    ...
    std::cout << "before PooledOrderBook::on_message() -> after PooledOrderBook::on_message()\n";
